...
<div id="wrap">
  ...
  <div id="divArticle">
    <article>
          ...
          <tr onclick="location.href='<c:url value="/qna/content?num=${qb.num}"/>'" id="atr">
            ...
            <td class="left">
              <c:if test="${qb.re_lev > 0 }">
                ...
                ${qb.subject }</td>
            <td>${qb.name }</td>
            <td><fmt:formatDate value="${qb.date}" type="both" pattern="yyyy.MM.dd HH:mm"/></td>
            <td>${qb.readcount }</td>
          </tr>
        </c:forEach>
      </table>
      ...
      <div class="clear"></div>
      <div id="table_search">
        <form action='<c:url value="/qna" />' method="get">
          <div id="divSearch">
      ...
      <div id="page_control">
        <c:if test="${pb.startPage > pb.pageBlock }">
          <a href='<c:url value="/qna?pageNum=${pb.startPage - pb.pageBlock }&search=${pb.search}" />'>이전</a>
        </c:if>
        <c:forEach var="i" begin="${pb.startPage }" end="${pb.endPage }" step="1">
          <a href='<c:url value="/qna?pageNum=${i }&search=${pb.search}" />'>${i }</a>
        </c:forEach>
        <c:if test="${pb.endPage < pb.pageCount }">
          <a href='<c:url value="/qna?pageNum=${pb.startPage + pb.pageBlock }&search=${pb.search}" />'>다음</a>
        </c:if>
      </div>
    </article>
  </div>
