    ...
    <article>
      ...
      <table id="notice">
        ...
        <c:forEach var="ab" items="${abList }">
          <tr onclick="location.href='<c:url value="/adopt/content?num=${ab.num}"/>'" id="atr">
            <td>${ab.num }</td>
            <td class="left">${ab.subject }</td>
            <td>${ab.name }</td>
            <td><fmt:formatDate value="${ab.date}" type="both" pattern="yyyy.MM.dd HH:mm"/></td>
            <td>${ab.readcount }</td>
          </tr>
        </c:forEach>
      </table>
      ...
      <div class="clear"></div>
      <div id="table_search">
        <form action='<c:url value="/adopt" />' method="get">
          <div id="divSearch">
      ...
      <div id="page_control">
        <c:if test="${pb.startPage > pb.pageBlock }">
          <a href='<c:url value="/adopt?pageNum=${pb.startPage - pb.pageBlock }&search=${pb.search}" />'>이전</a>
        </c:if>
        <c:forEach var="i" begin="${pb.startPage }" end="${pb.endPage }" step="1">
          <a href='<c:url value="/adopt?pageNum=${i }&search=${pb.search}" />'>${i }</a>
        </c:forEach>
        <c:if test="${pb.endPage < pb.pageCount }">
          <a href='<c:url value="/adopt?pageNum=${pb.startPage + pb.pageBlock }&search=${pb.search}" />'>다음</a>
        </c:if>
      </div>
    </article>
  </div>
