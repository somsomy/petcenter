...
<div id="wrap">
  ...
  <div id="divArticle">
    <article>
      ...
      <div class="clear"></div>
      <div id="table_search">
        <form action='<c:url value="/cats" />' method="get">
          <div id="divSearch">
      ...
      <div id="page_control">
        <c:if test="${pb.startPage > pb.pageBlock }">
          <a href='<c:url value="/cats?state=${param.state }&pageNum=${pb.startPage - pb.pageBlock }&search=${pb.search}" />'>이전</a>
        </c:if>
        <c:forEach var="i" begin="${pb.startPage }" end="${pb.endPage }" step="1">
          <a href='<c:url value="/cats?state=${param.state }&pageNum=${i }&search=${pb.search}" />'>${i }</a>
        </c:forEach>
        <c:if test="${pb.endPage < pb.pageCount }">
          <a href='<c:url value="/cats?state=${param.state }&pageNum=${pb.startPage + pb.pageBlock }&search=${pb.search}" />'>다음</a>
        </c:if>
      </div>
    </article>
  </div>
