  ...
  <article id="front">
          ...
          <c:forEach var="cb" items="${cbList }">
            <td>
              <table onclick="location.href='<c:url value="/cats/content?catId=${cb.catId}"/>'" class="ctable">
                <tr>
                  <td><img src='<c:url value="${cb.fileRealName }" />'/></td>
                </tr>
                <tr>
                  <td>${cb.catName }</td>
                </tr>
    ...
    <div id="page_control">
      <c:if test="${pb.startPage > pb.pageBlock }">
        <a href='<c:url value="/?pageNum=${pb.startPage - pb.pageBlock }&search=${pb.search}" />'>이전</a>
      </c:if>
      <c:forEach var="i" begin="${pb.startPage }" end="${pb.endPage }" step="1">
        <a href='<c:url value="/?pageNum=${i }&search=${pb.search}" />'>${i }</a>
      </c:forEach>
      <c:if test="${pb.endPage < pb.pageCount }">
        <a href='<c:url value="/?pageNum=${pb.startPage + pb.pageBlock }&search=${pb.search}" />'>다음</a>
      </c:if>
    </div>
  </article>
  <div class="clear"></div>
